...
int main()
{
    ...
    boost::shared_ptr<MyPath> stocks = pg.getPath();
    Real cumError = dh(*stocks);

    boost::shared_ptr<MyPath> options = dh.getOptionPrices();
    boost::shared_ptr<MyPath> deltas = dh.getDeltas();
    boost::shared_ptr<MyPath> errors = dh.getHedgingErrors();
    ...
    boost::shared_ptr<MyPath> stocks_csv = csvpg.getPath();
