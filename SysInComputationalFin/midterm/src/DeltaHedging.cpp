...
DeltaHedging::DeltaHedging(Option::Type type,
        ...
        : type_(type),
          ...
          outFile_(outFile),
          optionPrices_(new MyPath(nTimeSteps)),
          deltas_(new MyPath(nTimeSteps)),
          hedgingErrors_(new MyPath(nTimeSteps)),
          cash_(new MyPath(nTimeSteps)){
}
...
Real DeltaHedging::operator()(const MyPath& path) const {
...
boost::shared_ptr<MyPath> DeltaHedging::getOptionPrices(){
    return optionPrices_;
}

boost::shared_ptr<MyPath> DeltaHedging::getDeltas(){
    return deltas_;
}

boost::shared_ptr<MyPath> DeltaHedging::getHedgingErrors(){
    return hedgingErrors_;
}

boost::shared_ptr<MyPath> DeltaHedging::getCash(){
    return cash_;
}
...
void DeltaHedging::writeToCsv(const MyPath& path) const{
    return;
}
