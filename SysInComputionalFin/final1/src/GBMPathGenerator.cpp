...
boost::shared_ptr<MyPath<Real> > GBMPathGenerator::getPath(){
    ...
    boost::shared_ptr<MyPath<Real> > path(new MyPath<Real> (nTimeStep_, pathGen->next().value));
    //boost::shared_ptr<MyPath<Real> > MyPath<Real>  = boost::static_pointer_cast<MyPath<Real> >(path);

//    std::vector<Date> dates;
//    for (int i = 0; i < nTimeStep_; ++i){
//        dates.push_back(Date(1,Jan,2011));
//    }
//    MyPath<Real> ->setDates(dates);
    return path;
}
