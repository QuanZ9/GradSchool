#include "../include/DeltaHedgingDynamic.h"
...
DeltaHedgingDynamic::DeltaHedgingDynamic(Option::Type type,
        ...
        : DeltaHedging(type, strike, 0.24, getTime(startDate, expDate), getNTimeStep(startDate, endDate), 0.025, startCash, outFile),
          ...
          endDate_(endDate),
          rates_(new MyPath<Real> (getNTimeStep(startDate, endDate))),
          vols_(new MyPath<Real> (getNTimeStep(startDate, endDate))){
...
void DeltaHedgingDynamic::readRateFile(std::string filename){
    ...
    boost::shared_ptr<MyPath<Real> > path(new MyPath<Real> (nStep));
...
void DeltaHedgingDynamic::readOptionFile(std::string filename){
    ...
    Calendar calendar = UnitedStates();
    Integer nStep = calendar.businessDaysBetween(startDate_, endDate_, true, true);
    boost::shared_ptr<MyPath<Real> > vols(new MyPath<Real> (nStep));
    boost::shared_ptr<MyPath<Real> > options(new MyPath<Real> (nStep));
...
void DeltaHedgingDynamic::writeToCsv(const MyPath<Real> & path) const{
